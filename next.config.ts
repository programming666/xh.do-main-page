...
//
// `next dev` boots webpack's eval-based dev bundle, which the CSP below would
// otherwise block (see the script-src entry), so the policy is built per run
// phase. It is deliberately *not* keyed off NODE_ENV: `next build` only
// *defaults* NODE_ENV to "production" and happily picks up an exported value,
// so a shell or CI job with `NODE_ENV=development` would silently ship
// 'unsafe-eval' in the production policy. Production builds never use eval.
function buildPageCsp(isDev: boolean) {
  return [
    "default-src 'self'",
    "base-uri 'self'",
    "form-action 'self'",
    ...
    // `next dev` needs 'unsafe-eval': the dev bundle is built with
    // eval-source-map and react-refresh's runtime evaluates module code. Block
    // it and the dev page never hydrates at all (no interactivity, no client
    // effects — the hero keeps its server-rendered frame), which makes local
...
function createNextConfig(isDev: boolean): NextConfig {
  const pageCsp = buildPageCsp(isDev);
...
// Next calls the config with the run phase; `next dev` is the only phase that
// needs the eval relaxation.
export default function nextConfigWithIntl(phase: string) {
  return withNextIntl(createNextConfig(phase === PHASE_DEVELOPMENT_SERVER));
}
