      ...
      <div className="mx-auto max-w-4xl space-y-10">
        <div className="glass-panel rounded-[2rem] px-6 py-10 sm:px-10 sm:py-12">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <NextLink
              href={`/${locale}`}
              className="rounded-full border border-white/10 px-5 py-2.5 text-sm font-medium transition-colors hover:bg-white/5"
            >
              {t("backHome")}
            </NextLink>
          </div>
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2">
            {friendLinks.length ? friendLinks.map((item) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group flex gap-4 rounded-2xl border border-white/10 bg-black/10 p-5 transition-all hover:border-cyan-400/30 hover:bg-cyan-400/10 hover:shadow-[0_0_24px_rgba(76,201,255,0.12)]"
              >
                {item.imageUrl ? (
                  <Image
                    src={item.imageUrl}
                ...
                <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
                  <span className="text-lg font-medium transition-colors group-hover:text-cyan-300">
                    {item.label}
                  </span>
